...
class SimpleHybridTrainer(BaseTrainer):
    ...
    def setup_optimizer(self) -> Tuple[Optimizer, Optional[_LRScheduler]]:
        ...
        # 不同分支不同学习率（模型分支学习率更低）
        optimizer = Adam([
            {'params': data_params, 'lr': 5e-5, 'weight_decay': 1e-5},
            {'params': model_params, 'lr': 2e-5, 'weight_decay': 2e-5},  # 更低的学习率
            {'params': fusion_params, 'lr': 5e-5, 'weight_decay': 1e-5}
        ])
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        with tqdm(total=num_batches, desc=f"Epoch {self.current_epoch+1}") as pbar:
            for batch_idx, (noisy_imgs, clean_imgs, _) in enumerate(train_loader):
                ...
                # 计算损失
                loss = self.compute_loss_1(outputs, clean_imgs, losses_dict)
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        num_epochs = self.training_config.get('epochs',302)
        
        # 执行训练
        super().train(train_loader, val_loader, num_epochs)
