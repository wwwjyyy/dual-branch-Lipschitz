...
class DualBranchDenoise(nn.Module):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # 冻结状态跟踪
        self.data_branch_frozen = False
        self.model_branch_frozen = False
        self.fusion_frozen = False
    ...
    def forward(self, x: torch.Tensor, target: Optional[torch.Tensor] = None):
        ...
        if target is not None:
            # 主损失 - 融合输出
            losses_dict['fusion_loss'] = F.mse_loss(fused_output, target)
    ...
    def set_branch_trainability(self, trainable_branches: list, freeze_other: bool = True):
        """
        设置分支的可训练性
        
        Args:
            trainable_branches: 可训练的分支列表，如 ['model'], ['data'], 或 ['both']
            freeze_other: 是否冻结其他分支
        """
        # 确定各分支是否应该训练
        train_data = 'data' in trainable_branches or 'both' in trainable_branches
        train_model = 'model' in trainable_branches or 'both' in trainable_branches
        train_fusion = not freeze_other or 'both' in trainable_branches
        
        # 设置数据分支
        for param in self.data_branch.parameters():
            param.requires_grad = train_data
        self.data_branch_frozen = not train_data
        
        # 设置模型分支
        for param in self.model_branch.parameters():
            param.requires_grad = train_model
        self.model_branch_frozen = not train_model
        
        # 设置融合模块
        for param in self.fusion.parameters():
            param.requires_grad = train_fusion
        self.fusion_frozen = not train_fusion
        
        # 打印状态
        print(f"训练状态 - 数据分支: {'可训练' if train_data else '冻结'}, "
              f"模型分支: {'可训练' if train_model else '冻结'}, "
              f"融合模块: {'可训练' if train_fusion else '冻结'}")
    ...
    def get_branch_outputs(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            data_out = self.data_branch(x)
            model_out, _ = self.model_branch(x)
    ...
    def get_trainable_parameters_count(self) -> Dict[str, int]:
        """获取各分支的可训练参数数量"""
        data_params = sum(p.numel() for p in self.data_branch.parameters() if p.requires_grad)
        model_params = sum(p.numel() for p in self.model_branch.parameters() if p.requires_grad)
        fusion_params = sum(p.numel() for p in self.fusion.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        
        return {
            'data_branch': data_params,
            'model_branch': model_params,
