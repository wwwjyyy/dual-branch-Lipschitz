...
# 添加调试标志
DEBUG_LIPSCHITZ = False  # 设置为True启用详细调试输出
...
def compute_upper_bound(
    ...
    def _process_with_device_unification(obj, dtype):
        ...
        if isinstance(obj, list):
            ...
            # 确保所有元素都在CPU上并计算乘积
            total = torch.tensor(1.0, dtype=dtype, device='cpu')
            for elem in elements:
                if isinstance(elem, list):
                    # 如果元素是列表，递归计算
                    sub_total = compute_upper_bound(elem, dtype)
                    total = total * sub_total
                elif isinstance(elem, torch.Tensor):
                    total = total * elem.cpu().type(dtype)
                else:
                    total = total * torch.tensor(elem, dtype=dtype, device='cpu')
            
            debug_print(f"列表所有元素的乘积: {total}")
            return total
...
def compute_lipschitz_upper_bound_per_layer(
    ...
    # 首先检查是否为1-Lipschitz激活函数或池化层
    if isinstance(layer, (nn.ReLU, nn.Sigmoid, nn.Tanh, nn.LeakyReLU)):
        debug_print(f"检测到 1-Lipschitz 激活函数: {type(layer).__name__}")
        return torch.tensor(1.0, dtype=dtype)
    
    if isinstance(layer, (nn.AdaptiveAvgPool2d, nn.AdaptiveMaxPool2d, nn.AvgPool2d, nn.MaxPool2d)):
        debug_print(f"检测到 1-Lipschitz 池化层: {type(layer).__name__}")
        return torch.tensor(1.0, dtype=dtype)
    ...
    if isinstance(layer, nn.Sequential):
        ...
        # 改进的Sequential处理：正确处理输入形状
        if isinstance(layer_input_shape, list) and len(layer_input_shape) > 0 and not isinstance(layer_input_shape[0], int):
            # 如果提供了每个层的输入形状列表
            if len(layer_input_shape) == len(layer):
                debug_print("使用逐个层的输入形状")
                result = [
                    compute_lipschitz_upper_bound_per_layer(
                        layer[i], layer_input_shape[i], dtype, fast_approximation, ord)
                    for i in range(len(layer))
                ]
            else:
                # 如果形状数量不匹配，使用第一个形状给所有层
                debug_print("形状数量不匹配，使用第一个形状给所有层")
                result = [
                    compute_lipschitz_upper_bound_per_layer(
                        layer[i], layer_input_shape[0], dtype, fast_approximation, ord)
                    for i in range(len(layer))
                ]
        else:
            ...
            result = [
                compute_lipschitz_upper_bound_per_layer(
                    layer[i], layer_input_shape, dtype, fast_approximation, ord)
                for i in range(len(layer))
            ]
    ...
    if isinstance(layer, resnet.BasicBlock) or isinstance(layer, resnet.Bottleneck):
        ...
        lip = [compute_lipschitz_upper_bound_per_layer(
            layer.conv1, layer_input_shape[0], dtype, fast_approximation, ord)]
        debug_print(f"conv1 Lipschitz: {lip[-1]}")
        
        # // out = self.bn1(out)
        lip += [compute_lipschitz_upper_bound_per_layer(layer.bn1, [], dtype, fast_approximation, ord)]
        ...
        # // out = self.conv2(out)
        lip += [compute_lipschitz_upper_bound_per_layer(
            layer.conv2, layer_input_shape[1], dtype, fast_approximation, ord)]
        debug_print(f"conv2 Lipschitz: {lip[-1]}")
        
        # // out = self.bn2(out)
        lip += [compute_lipschitz_upper_bound_per_layer(layer.bn2, [], dtype, fast_approximation, ord)]
        ...
        if isinstance(layer, resnet.Bottleneck):
            ...
            lip += [
                compute_lipschitz_upper_bound_per_layer(
                    layer.conv3, layer_input_shape[2], dtype, fast_approximation, ord)
            ]
            debug_print(f"conv3 Lipschitz: {lip[-1]}")
            
            # // out = self.bn3(out)
            lip += [compute_lipschitz_upper_bound_per_layer(
                layer.bn3, [], dtype, fast_approximation, ord)]
        ...
        if layer.downsample is not None:
            ...
            lip_residual = compute_lipschitz_upper_bound_per_layer(
                layer.downsample, [layer_input_shape[0], []], dtype, fast_approximation, ord
            )
...
def compute_dual_branch_lipschitz(layer, layer_input_shape, dtype, ord):
    ...
    try:
        ...
        if isinstance(lip_data, list):
            data_bound = compute_upper_bound(lip_data, dtype)
            debug_print(f"数据分支列表上界: {data_bound}")
        else:
            data_bound = lip_data.cpu() if isinstance(lip_data, torch.Tensor) else torch.tensor(lip_data, dtype=dtype, device='cpu')
            debug_print(f"数据分支张量上界: {data_bound}")
        
        # 处理模型分支的结果
        if isinstance(lip_model, list):
            model_bound = compute_upper_bound(lip_model, dtype)
            debug_print(f"模型分支列表上界: {model_bound}")
        else:
            model_bound = lip_model.cpu() if isinstance(lip_model, torch.Tensor) else torch.tensor(lip_model, dtype=dtype, device='cpu')
            debug_print(f"模型分支张量上界: {model_bound}")
        
        # 处理融合层的结果
        if isinstance(lip_fusion, list):
            fusion_bound = compute_upper_bound(lip_fusion, dtype)
            debug_print(f"融合层列表上界: {fusion_bound}")
        else:
            fusion_bound = lip_fusion.cpu() if isinstance(lip_fusion, torch.Tensor) else torch.tensor(lip_fusion, dtype=dtype, device='cpu')
            debug_print(f"融合层张量上界: {fusion_bound}")
...
def compute_data_driven_branch_lipschitz(layer, layer_input_shape, dtype, ord):
    ...
    try:
        ...
        # 处理encoder部分
        if hasattr(layer, 'encoder') and layer.encoder is not None:
            debug_print("处理encoder部分")
            # 确保输入形状正确传递
            if isinstance(layer_input_shape, list) and len(layer_input_shape) > 0:
                encoder_input_shape = layer_input_shape[0] if isinstance(layer_input_shape[0], list) else layer_input_shape
            else:
                encoder_input_shape = layer_input_shape
                
            lip_encoder = compute_lipschitz_upper_bound_per_layer(
                layer.encoder, encoder_input_shape, dtype, ord)
            lip_constants.append(lip_encoder)
            debug_print(f"encoder Lipschitz: {lip_encoder}")
        
        # 处理attention部分
        if hasattr(layer, 'attention') and layer.attention is not None:
            debug_print("处理attention部分")
            # attention的输入形状与encoder输出相同
            attention_input_shape = [64, 256, 256]  # 根据SimpleDataDrivenBranch结构设定
            lip_attention = compute_lipschitz_upper_bound_per_layer(
                layer.attention, attention_input_shape, dtype, ord)
            lip_constants.append(lip_attention)
            debug_print(f"attention Lipschitz: {lip_attention}")
        
        # 处理decoder部分
        if hasattr(layer, 'decoder') and layer.decoder is not None:
            debug_print("处理decoder部分")
            # decoder的输入形状与attention输出相同
            decoder_input_shape = [64, 256, 256]  # 根据SimpleDataDrivenBranch结构设定
            lip_decoder = compute_lipschitz_upper_bound_per_layer(
                layer.decoder, decoder_input_shape, dtype, ord)
            lip_constants.append(lip_decoder)
            debug_print(f"decoder Lipschitz: {lip_decoder}")
        
        debug_print(f"数据驱动分支 Lipschitz 组件: {lip_constants}")
        ...
        for lip in lip_constants:
            if isinstance(lip, list):
                ...
                lip_bound = lip if isinstance(lip, torch.Tensor) else torch.tensor(lip, dtype=dtype)
            total_bound = total_bound * lip_bound
        
        debug_print(f"数据驱动分支总 Lipschitz: {total_bound}")
        return total_bound
